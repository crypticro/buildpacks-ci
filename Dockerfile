...
# Install go 1.12
RUN cd /usr/local \
  && curl -L https://dl.google.com/go/go1.12.linux-amd64.tar.gz -o go.tar.gz \
  && [ 750a07fef8579ae4839458701f4df690e0b20b8bcce33b437e4df89c451b6f13 = $(shasum -a 256 go.tar.gz | cut -d' ' -f1) ] \
  && tar xf go.tar.gz \
  && rm go.tar.gz \
